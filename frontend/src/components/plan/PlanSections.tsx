...
export function BudgetTab({ plan }: { plan: FullPlan }) {
  ...
  const data = ep.budget.byCategory.map((c) => ({ name: c.category, value: Math.round(c.amountUSD * rate) }));
        ...
        <div className="rounded-xl border border-border bg-card/40 p-5 h-72" data-print-card data-print-chart-hide>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip formatter={(v: number) => `${symbol}${v.toLocaleString()}`} contentStyle={{ background: "#1a2238", border: "1px solid #334" }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
        <div className="space-y-3">
          {data.map((d, i) => (
            <div key={d.name} className="flex items-center justify-between p-3 rounded-lg border border-border bg-card/40">
              <div className="flex items-center gap-3">
                <span className="h-3 w-3 rounded-full" style={{ background: COLORS[i] }} />
                <span className="text-sm">{d.name}</span>
              </div>
              <span className="font-mono">{symbol}{d.value.toLocaleString()}</span>
            </div>
          ))}
          <div className="flex items-center justify-between p-3 rounded-lg border border-amber-500/30 bg-amber-500/5">
            <span className="text-sm text-amber-300">Contingency ({ep.budget.contingencyPercent}%)</span>
            <span className="font-mono text-amber-300">{symbol}{contingency.toLocaleString()}</span>
          </div>
          <div className="flex items-center justify-between p-4 rounded-lg border border-primary/40 bg-primary/5">
            <span className="font-semibold">Total</span>
            <span className="font-mono font-display text-2xl text-gradient">{symbol}{grandTotal.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      >
        <div className="space-y-2">{v.failureModes.map((f, i) => (
          <div key={i} className="rounded-md border border-border bg-card/30 p-3">
            ...
            <p className="text-xs text-muted-foreground mt-1"><span className="text-amber-400">Early detection:</span> {f.earlyDetection}</p>
          </div>
        ))}</div>
      </Section>
      <Section
        ...
        <div className="space-y-3">{s.hazardousMaterials.map((m, i) => (
          <div key={i} className="rounded-md border border-rose-500/30 bg-rose-500/5 p-3" data-print-card>
            <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
              ...
              <div className="flex gap-1">{m.ghsSymbols.map((g) => <Badge key={g} variant="outline" className="border-rose-500/40 bg-rose-500/10 text-rose-200 text-xs font-mono">{g}</Badge>)}</div>
            </div>
            <ul className="text-xs text-foreground/80 space-y-0.5">{m.hazards.map((h, j) => <li key={j}>• {h}</li>)}</ul>
          </div>
        ))}</div>
      </Section>
      <Section
        ...
        <div>
          ...
          <div className="space-y-2">{plan.reasoning.confidence.map((c, i) => (
            <div key={i} className="flex items-start justify-between gap-3 text-sm">
              ...
              <Badge className={c.level === "High" ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/30" : c.level === "Medium" ? "bg-amber-500/15 text-amber-300 border-amber-500/30" : "bg-rose-500/15 text-rose-300 border-rose-500/30"}>{c.level}</Badge>
            </div>
          ))}</div>
        </div>
      </div>
