...
export function MaterialsTab({ plan }: { plan: FullPlan }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border bg-card/40 backdrop-blur overflow-hidden" data-print-card>
                ...
                return (
                  <tr key={i} className="hover:bg-card/60 transition-colors">
                    <td className="px-3 py-3 align-top">
                      ...
                      <Badge variant="outline" className="mt-1 text-[10px] uppercase tracking-wider border-border bg-card/50">
                        {m.category}
                      </Badge>
                    </td>
                    ...
                    <td className="px-3 py-3 align-top">
                      ...
                      <span className="block text-[10px] text-amber-400/80 mt-0.5" title="Verify current catalog and pricing before ordering">⚠ Verify before ordering</span>
                    </td>
                    ...
                    <td className="px-3 py-3 align-top text-right">
                      <span className={longLead ? "inline-flex items-center gap-1 text-amber-400 font-mono text-xs" : "text-muted-foreground font-mono text-xs"}>
...
function SubCard({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-lg border p-4 ${highlight ? "border-primary/40 bg-primary/5" : "border-border bg-card/40"}`} data-print-card>
      <p className="text-xs uppercase tracking-widest text-muted-foreground mb-1">{label}</p>
      <p className={`font-display font-semibold ${highlight ? "text-2xl text-gradient" : "text-xl"}`}>
